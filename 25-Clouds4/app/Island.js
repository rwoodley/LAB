function getHeightData(img, worldWidth, worldDepth) {
    ...
    for (var i = 0, n = pix.length; i < n; i += (4)) {
        ...
        data[j] = 20 * all/30;	// 50 is the z resolution.
...
function generateTexture( indata, width, height, light ) {
	...
	sun = new THREE.Vector3( light.position.x, light.position.y, light.position.z );
	sun.normalize();
	...
	for ( var i = 0, j = 0, l = imageData.length; i < l; i += 4, j ++ ) {
		...
		imageData[ i ] = ( 96 + shade * 228 ) * ( 0.5 + data[ j ] * 0.007 );
		imageData[ i + 1 ] = ( 32 + shade * 196 ) * ( 0.5 + data[ j ] * 0.007 );
