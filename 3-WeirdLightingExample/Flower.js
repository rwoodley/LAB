/*
This shows a weird example.
2 flowers, both lit from the same spot light, but morphed materials that don't morph. (MorphInfluence hard-coded to 0).
But one is correctly lit (metalic where the light hits it), and one is only metallic inside the cup.
The 2nd one is wrong, but it is a cool effect. It seems to take the under-surface from the lighting of
the eventual, but never realized morph surface.
 */
...
function init() {
	...
	//}
	var flower = drawCube(0,0,0, funcAlmost, funcStart);
	var flower2 = drawCube(-20,0,-20, funcDiff, funcStart   );
...
function drawCube(x,y,z, func1, func2) {
	var geometry = new THREE.ParametricGeometry(func1, 60, 60, false);
	var geometry2 = new THREE.ParametricGeometry(func2, 60, 60, false);
